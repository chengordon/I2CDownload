...
namespace I2CDownload
{
    public class ClsBaseCDB
    {
        ...
        public byte gbytFormTableSel = 0x2E;//Form读取的IIC选表
        public static byte Table_DSP_TXH0_7Fir_Temp = 0xE5;//Form读取的IIC选表
        public static byte Table_DSP_TXH0_7Fir = 0xED;

        public string strAPP = "";

        public byte gbytMediaInterface = 0x0C;

        public static byte bytConfigDSPMode = 1;
        public static byte bytConfigAutoCTLE = 0;
        public byte gbytConfigAFETrim = 2;
        public static byte bytConfigMLSD = 0;
        public byte gbytConfigSCDREN = 0;
        public static byte bytConfigMSEMinThreshold = 91;
        public byte gbytConfigCDRParams = 0;
        public byte gbytConfigSCDRMulSel = 3;

        public int  gintFlashDelayTime = 100;//最小130ms,120ms验证MCU&DSP combine bin有问题
        public byte gbytIICDelayTime = 2;
        public byte gbytBinType = 0;//Bin文件类型(0:单独的;1:合并的)
        ...
        CH341Library.CH341_Device CH341Device = new CH341Library.CH341_Device();
        ...
        public bool SetCDBPW(byte[] bytPW)
        {
        ...
        public string GetModuleRev()
        {
            ...
            if (EnterCDBPW() == false || WriteTableSel((byte)gbytRevTableSel) == false || ReadBytes(gbytPasswordDevAddr, gbytRevRegAddr, gbytRevLen, bytTemp) == false) return strTemp;
            for (int i = 0; i < gbytRevLen; i++)
            {
                strTemp += bytTemp[i].ToString("X2");
            }
            return strTemp;
        }
        ...
        {
            string strTemp = "";
            byte[] RBuf = new byte[22];
            if (EnterCDBPW() == false) return strTemp;
            if (WriteTableSel((byte)gbytFormTableSel) == false) return Encoding.ASCII.GetString(RBuf);

            if (ReadBytes(gbytPasswordDevAddr, 128, 8, RBuf) == false) return Encoding.ASCII.GetString(RBuf);
            strTemp = Encoding.ASCII.GetString(RBuf).Replace("\0", "");

            Array.Clear(RBuf, 0, RBuf.Length);
            if (ReadBytes(gbytPasswordDevAddr, 136, 8, RBuf) == false) return Encoding.ASCII.GetString(RBuf);
            strTemp += Encoding.ASCII.GetString(RBuf).Replace("\0", "");

            Array.Clear(RBuf, 0, RBuf.Length);
            if (ReadBytes(gbytPasswordDevAddr, 152, 8, RBuf) == false) return Encoding.ASCII.GetString(RBuf);
            strTemp += Encoding.ASCII.GetString(RBuf).Replace("\0", "");
            return strTemp;
        }

        public bool WriteModuleAPP()
        {
            string strTemp = strAPP;
            int bCheckSum = 0;
            byte[] bytAPPBuf = new byte[64];
            byte[] bytReadBuf = new byte[128];
            byte[] bytTempBuf = new byte[128];
            ...
            {
                bytAPPBuf[i] = Convert.ToByte(strTemp.Substring(i * 2, 2).Trim(), 16);
            }

            if (EnterCDBPW() == false || WriteTableSel(0) == false) return false;
            DelayMS(5);
            if (WriteBytes(gbytPasswordDevAddr, 85, 33, bytAPPBuf) == false) return false;
            DelayMS(gintFlashDelayTime);
            if (ReadBytes(gbytPasswordDevAddr, 85, 33, bytTempBuf) == false) return false;
            for (int i = 0; i < 33; i++)
            {
                if (bytTempBuf[i] != bytAPPBuf[i])
                {
                    return false;
                }
            }

            if (ReadBytes(gbytPasswordDevAddr, 128, 128, bytReadBuf) == false) return false;

            bytReadBuf[212 - 128] = gbytMediaInterface;

            for (int j = 0; j < 222 - 128; j++)
            {
                bCheckSum += bytReadBuf[j];
            }

            bytReadBuf[222 - 128] = (byte)bCheckSum;

            DelayMS(5);
            if (WriteBytes(gbytPasswordDevAddr, 128, 128, bytReadBuf) == false) return false;
            DelayMS(gintFlashDelayTime);

            if (ReadBytes(gbytPasswordDevAddr, 128, 128, bytTempBuf) == false) return false;
            for (int i = 0; i < 128; i++)
            {
                if (bytTempBuf[i] != bytReadBuf[i])
                {
                    return false;
                }
            }

            if (WriteTableSel(1) == false || ReadBytes(gbytPasswordDevAddr, 128, 128, bytReadBuf) == false) return false;

            bytReadBuf[176 - 128] = bytAPPBuf[4];//App1
            bytReadBuf[177 - 128] = bytAPPBuf[8];//App2
            bytReadBuf[178 - 128] = bytAPPBuf[12];//App3
            bytReadBuf[179 - 128] = bytAPPBuf[16];//App4
            bytReadBuf[180 - 128] = bytAPPBuf[20];//App5
            bytReadBuf[181 - 128] = bytAPPBuf[24];//App6
            bytReadBuf[182 - 128] = bytAPPBuf[28];//App7
            bytReadBuf[183 - 128] = bytAPPBuf[32];//App8

            for (int j = 2; j < 127; j++)
            {
                bCheckSum += bytReadBuf[j];
            }
            bytReadBuf[127] = (byte)bCheckSum;
            DelayMS(5);
            if (WriteBytes(gbytPasswordDevAddr, 128, 128, bytReadBuf) == false) return false;
            DelayMS(gintFlashDelayTime);
        ...
        public bool WriteTXH0_7Config()
        {
            byte[] bytReadBuf = new byte[128];
            byte[] bytTempBuf = new byte[128];
            byte bytTemp = 0;

            if (EnterCDBPW() == false || WriteTableSel(Table_DSP_TXH0_7Fir_Temp) == false ||
                ReadBytes(gbytPasswordDevAddr, 128, 128, bytReadBuf) == false) return false;

            #region DSPMode
            if (bytConfigDSPMode == 0x00 || bytConfigDSPMode == 0x01 || bytConfigDSPMode == 0x04 || bytConfigDSPMode == 0x05)
            {
                bytTemp = bytConfigDSPMode;
            }
            else
            {
                bytTemp = 0x01;
            ...
            ////Port0
            //if (!WriteBytes(gbytPasswordDevAddr, 128, 1, bytWBuf)) return false;
            ////Port1
            //if (!WriteBytes(gbytPasswordDevAddr, 144, 1, bytWBuf)) return false;
            ////Port2
            //if (!WriteBytes(gbytPasswordDevAddr, 160, 1, bytWBuf)) return false;
            ////Port3
            //if (!WriteBytes(gbytPasswordDevAddr, 176, 1, bytWBuf)) return false;
            ////Port4
            //if (!WriteBytes(gbytPasswordDevAddr, 192, 1, bytWBuf)) return false;
            ////Port5
            //if (!WriteBytes(gbytPasswordDevAddr, 208, 1, bytWBuf)) return false;
            ////Port6
            //if (!WriteBytes(gbytPasswordDevAddr, 224, 1, bytWBuf)) return false;
            ////Port7
            //if (!WriteBytes(gbytPasswordDevAddr, 240, 1, bytWBuf)) return false;

            for (int i = 0; i < 8; i++)
            {
                bytReadBuf[128 - 128 + i * 16] = bytTemp;
            }
            #endregion

            #region AutoCTLE
            if (bytConfigAutoCTLE == 0x01)
            {
                bytTemp = 0x01;
            }
            else
            ...
            ////Port0
            //if (!WriteBytes(gbytPasswordDevAddr, 131, 1, bytWBuf)) return false;
            ////Port1
            //if (!WriteBytes(gbytPasswordDevAddr, 147, 1, bytWBuf)) return false;
            ////Port2
            //if (!WriteBytes(gbytPasswordDevAddr, 163, 1, bytWBuf)) return false;
            ////Port3
            //if (!WriteBytes(gbytPasswordDevAddr, 179, 1, bytWBuf)) return false;
            ////Port4
            //if (!WriteBytes(gbytPasswordDevAddr, 195, 1, bytWBuf)) return false;
            ////Port5
            //if (!WriteBytes(gbytPasswordDevAddr, 211, 1, bytWBuf)) return false;
            ////Port6
            //if (!WriteBytes(gbytPasswordDevAddr, 227, 1, bytWBuf)) return false;
            ////Port7
            //if (!WriteBytes(gbytPasswordDevAddr, 243, 1, bytWBuf)) return false;

            for (int i = 0; i < 8; i++)
            {
                bytReadBuf[131 - 128 + i * 16] = bytTemp;
            }
            #endregion

            #region AFETrim
            if (gbytConfigAFETrim == 0x01 || gbytConfigAFETrim == 0x02 || gbytConfigAFETrim == 0x03 || gbytConfigAFETrim == 0x04 ||
                gbytConfigAFETrim == 0x05 || gbytConfigAFETrim == 0x06 || gbytConfigAFETrim == 0x07 || gbytConfigAFETrim == 0x08)
            {
                bytTemp = gbytConfigAFETrim;
            }
            else
            {
                bytTemp = 0x01;
            ...
            ////Port0
            //if (!WriteBytes(gbytPasswordDevAddr, 134, 1, bytWBuf)) return false;
            ////Port1
            //if (!WriteBytes(gbytPasswordDevAddr, 150, 1, bytWBuf)) return false;
            ////Port2
            //if (!WriteBytes(gbytPasswordDevAddr, 166, 1, bytWBuf)) return false;
            ////Port3
            //if (!WriteBytes(gbytPasswordDevAddr, 182, 1, bytWBuf)) return false;
            ////Port4
            //if (!WriteBytes(gbytPasswordDevAddr, 198, 1, bytWBuf)) return false;
            ////Port5
            //if (!WriteBytes(gbytPasswordDevAddr, 214, 1, bytWBuf)) return false;
            ////Port6
            //if (!WriteBytes(gbytPasswordDevAddr, 230, 1, bytWBuf)) return false;
            ////Port7
            //if (!WriteBytes(gbytPasswordDevAddr, 246, 1, bytWBuf)) return false;

            for (int i = 0; i < 8; i++)
            {
                bytReadBuf[134 - 128 + i * 16] = bytTemp;
            }
            #endregion

            #region MLSD
            if (bytConfigMLSD == 0x01)
            {
                bytTemp = 0x01;
            }
            else
            ...
            ////Port0
            //if (!WriteBytes(gbytPasswordDevAddr, 136, 1, bytWBuf)) return false;
            ////Port1
            //if (!WriteBytes(gbytPasswordDevAddr, 152, 1, bytWBuf)) return false;
            ////Port2
            //if (!WriteBytes(gbytPasswordDevAddr, 168, 1, bytWBuf)) return false;
            ////Port3
            //if (!WriteBytes(gbytPasswordDevAddr, 184, 1, bytWBuf)) return false;
            ////Port4
            //if (!WriteBytes(gbytPasswordDevAddr, 200, 1, bytWBuf)) return false;
            ////Port5
            //if (!WriteBytes(gbytPasswordDevAddr, 216, 1, bytWBuf)) return false;
            ////Port6
            //if (!WriteBytes(gbytPasswordDevAddr, 232, 1, bytWBuf)) return false;
            ////Port7
            //if (!WriteBytes(gbytPasswordDevAddr, 248, 1, bytWBuf)) return false;

            for (int i = 0; i < 8; i++)
            {
                bytReadBuf[136 - 128 + i * 16] = bytTemp;
            }
            #endregion

            #region SCDREN
            if (gbytConfigSCDREN == 0x01)
            {
                bytTemp = 0x01;
            }
            else
            ...
            ////Port0
            //if (!WriteBytes(gbytPasswordDevAddr, 138, 1, bytWBuf)) return false;
            ////Port1
            //if (!WriteBytes(gbytPasswordDevAddr, 154, 1, bytWBuf)) return false;
            ////Port2
            //if (!WriteBytes(gbytPasswordDevAddr, 170, 1, bytWBuf)) return false;
            ////Port3
            //if (!WriteBytes(gbytPasswordDevAddr, 186, 1, bytWBuf)) return false;
            ////Port4
            //if (!WriteBytes(gbytPasswordDevAddr, 202, 1, bytWBuf)) return false;
            ////Port5
            //if (!WriteBytes(gbytPasswordDevAddr, 218, 1, bytWBuf)) return false;
            ////Port6
            //if (!WriteBytes(gbytPasswordDevAddr, 234, 1, bytWBuf)) return false;
            ////Port7
            //if (!WriteBytes(gbytPasswordDevAddr, 250, 1, bytWBuf)) return false;

            for (int i = 0; i < 8; i++)
            {
                bytReadBuf[138 - 128 + i * 16] = bytTemp;
            }
            #endregion

            #region MSEMinThreshold
            if (bytConfigMSEMinThreshold < 128)
            {
                bytTemp = bytConfigMSEMinThreshold;
            }
            else
            {
                bytTemp = 91;
            ...
            ////Port0
            //if (!WriteBytes(gbytPasswordDevAddr, 141, 1, bytWBuf)) return false;
            ////Port1
            //if (!WriteBytes(gbytPasswordDevAddr, 157, 1, bytWBuf)) return false;
            ////Port2
            //if (!WriteBytes(gbytPasswordDevAddr, 173, 1, bytWBuf)) return false;
            ////Port3
            //if (!WriteBytes(gbytPasswordDevAddr, 189, 1, bytWBuf)) return false;
            ////Port4
            //if (!WriteBytes(gbytPasswordDevAddr, 205, 1, bytWBuf)) return false;
            ////Port5
            //if (!WriteBytes(gbytPasswordDevAddr, 221, 1, bytWBuf)) return false;
            ////Port6
            //if (!WriteBytes(gbytPasswordDevAddr, 237, 1, bytWBuf)) return false;
            ////Port7
            //if (!WriteBytes(gbytPasswordDevAddr, 253, 1, bytWBuf)) return false;

            for (int i = 0; i < 8; i++)
            {
                bytReadBuf[141 - 128 + i * 16] = bytTemp;
            }
            #endregion

            #region CDRParams
            if (gbytConfigCDRParams == 0x00 || gbytConfigCDRParams == 0x01 || gbytConfigCDRParams == 0x02 || gbytConfigCDRParams == 0x03 || gbytConfigCDRParams == 0x04)
            {
                bytTemp = gbytConfigCDRParams;
            }
            else
            {
                bytTemp = 0;
            ...
            ////Port0
            //if (!WriteBytes(gbytPasswordDevAddr, 142, 1, bytWBuf)) return false;
            ////Port1
            //if (!WriteBytes(gbytPasswordDevAddr, 158, 1, bytWBuf)) return false;
            ////Port2
            //if (!WriteBytes(gbytPasswordDevAddr, 174, 1, bytWBuf)) return false;
            ////Port3
            //if (!WriteBytes(gbytPasswordDevAddr, 190, 1, bytWBuf)) return false;
            ////Port4
            //if (!WriteBytes(gbytPasswordDevAddr, 206, 1, bytWBuf)) return false;
            ////Port5
            //if (!WriteBytes(gbytPasswordDevAddr, 222, 1, bytWBuf)) return false;
            ////Port6
            //if (!WriteBytes(gbytPasswordDevAddr, 238, 1, bytWBuf)) return false;
            ////Port7
            //if (!WriteBytes(gbytPasswordDevAddr, 254, 1, bytWBuf)) return false;

            for (int i = 0; i < 8; i++)
            {
                bytReadBuf[142 - 128 + i * 16] = bytTemp;
            }
            #endregion

            #region SCDRMulSel
            if (gbytConfigSCDRMulSel < 8)
            {
                bytTemp = gbytConfigSCDRMulSel;
            }
            else
            {
                bytTemp = 0;
            ...
            ////Port0
            //if (!WriteBytes(gbytPasswordDevAddr, 143, 1, bytWBuf)) return false;
            ////Port1
            //if (!WriteBytes(gbytPasswordDevAddr, 159, 1, bytWBuf)) return false;
            ////Port2
            //if (!WriteBytes(gbytPasswordDevAddr, 175, 1, bytWBuf)) return false;
            ////Port3
            //if (!WriteBytes(gbytPasswordDevAddr, 191, 1, bytWBuf)) return false;
            ////Port4
            //if (!WriteBytes(gbytPasswordDevAddr, 207, 1, bytWBuf)) return false;
            ////Port5
            //if (!WriteBytes(gbytPasswordDevAddr, 223, 1, bytWBuf)) return false;
            ////Port6
            //if (!WriteBytes(gbytPasswordDevAddr, 239, 1, bytWBuf)) return false;
            ////Port7
            //if (!WriteBytes(gbytPasswordDevAddr, 255, 1, bytWBuf)) return false;

            for (int i = 0; i < 8; i++)
            {
                bytReadBuf[143 - 128 + i * 16] = bytTemp;
            }
            #endregion

            if (WriteTableSel(Table_DSP_TXH0_7Fir) == false ) return false;
            DelayMS(5);
            if (WriteBytes(gbytPasswordDevAddr, 128, 128, bytReadBuf) == false) return false;
            DelayMS(gintFlashDelayTime);
        ...
        private bool GetCDBStatus(ref string errorStr, string errorMsg = "")
        {
            ...
            {
                if (ReadBytes(gbytPasswordDevAddr, 37, 2, tablesel) == false)
                {
                    if(gbytBinType!=0)
                    {
                        int j;
                        //延时再读的原因是为解决combine MCU&FW bin在下载完MCU bin有收尾工作
                        for (j = 20; j > 0; j--)
                        {
                            DelayMS(50);
                            if (ReadBytes(gbytPasswordDevAddr, 37, 2, tablesel) == true) break;
                        }
        ...
        private bool Read_StatusData(byte[] bytData, byte DataLen = 120)
        {
            byte bytCheckSum = 0;
            byte UpdateDevAddr = (byte)gbytPasswordDevAddr;
            byte[] bytDataBuf = new byte[BLOCKSIZE];
        ...
        private bool SendCMD_Start(byte[] bytData, UInt32 FileLen, byte DataLen)
        {
            byte[] bytDataBuf = new byte[BLOCKSIZE];
        ...
        private bool SendCMD_EPLRead(uint BlockAddress, ushort Length)
        {
            ...
            bytDataBuf[130 - 128] = (byte)((Length >> 8) & 0xFF);
            bytDataBuf[131 - 128] = (byte)(Length & 0xFF);
            bytDataBuf[132 - 128] = 0x06;
            bytDataBuf[136 - 128] = (byte)((BlockAddress >> 24) & 0xFF);
            bytDataBuf[137 - 128] = (byte)((BlockAddress >> 16) & 0xFF);
            bytDataBuf[138 - 128] = (byte)((BlockAddress >> 8) & 0xFF);
        ...
        private bool ReadEPLData(byte[] bytData, ushort DataLen)
        {
            byte bytCheckSum = 0, bytCheckCode = 0, iReadNum = BLOCKSIZE;
            byte UpdateDevAddr = (byte)gbytPasswordDevAddr;
            byte[] bytDataBuf = new byte[BLOCKSIZE];
            uint iCurReadNum = DataLen;

            //选取数据表
            if (WriteTableSel(gbytCDBCMDTableSel) == false) return false;
            if (ReadBytes(UpdateDevAddr, 0x80, 8, bytDataBuf) == false) return false;

            bytCheckCode = bytDataBuf[135 - 128];
            ...
            for (int i = 0; i < ((iCurReadNum - 1) / BLOCKSIZE + 1); i++)
            {
                ...
                {
                    iReadNum = (byte)(iCurReadNum - i * BLOCKSIZE);
                }
                ...
                if (ReadBytes(UpdateDevAddr, 0x80, iReadNum, bytDataBuf) == false) return false;

                Array.Copy(bytDataBuf, 0, bytData, i * BLOCKSIZE, iReadNum);
            }
        ...
        public bool ReadStatusData(byte[] bytData, byte DataLen, ref string errorStr, string errorMsg = "")
        {
            if (Read_StatusData(bytData, DataLen) == false || GetCDBStatus(ref errorStr, errorMsg) == false)
            {
                return false;
            }
            else
        ...
        public bool SendCDBStart(byte[] bytData, uint FileLen, byte DataLen, ref string errorStr, string errorMsg = "")
        {
        ...
        public bool SendCDBAbort(ref string errorStr, string errorMsg = "")
        {
            if (SendCMD_Abort() == false || GetCDBStatus(ref errorStr, errorMsg) == false)
            {
                return false;
            }
            else
        ...
        public bool WriteDataLPL(uint FlashAddr, byte[] bytData, uint DataLen, ref string errorStr, string errorMsg = "")
        {
            uint iIndex = 0, iAddr = FlashAddr, iCurWriteNum = DataLen;
            byte[] bytDataBuf = new byte[BLOCKSIZE];
        ...
        public bool WriteDataEPL(uint FlashAddr, byte[] bytData, uint DataLen, ref string errorStr, string errorMsg = "")
        {
            uint iIndex = 0, iAddr = FlashAddr, iCurWriteNum = DataLen, iWriteNum = BLOCKSIZE;
            byte[] bytDataBuf = new byte[BLOCKSIZE];
            ...
            while (DataLen > 0)
            {
                ...
                for (int i = 0; i < ((iCurWriteNum - 1) / BLOCKSIZE + 1); i++)
                {
                    ...
                    {
                        iWriteNum = (uint)(iCurWriteNum - i * BLOCKSIZE);
                    }
                    ...
                    if (WriteBytes(gbytPasswordDevAddr, 0x80, (int)iWriteNum, bytDataBuf) == false) return false;
                }
        ...
        public bool ReadDataLPL(uint FlashAddr, byte[] bytData, uint DataLen, ref string errorStr, string errorMsg = "")
        {
            uint iIndex = 0, iAddr = FlashAddr, iCurReadNum = DataLen;
            byte[] bytDataBuf = new byte[gbytCDBLPLDataSize];
        ...
        public bool ReadDataEPL(uint FlashAddr, byte[] bytData, uint DataLen, ref string errorStr, string errorMsg = "")
        {
            uint iIndex = 0, iAddr = FlashAddr, iCurReadNum = DataLen;
            byte[] bytDataBuf = new byte[gwCDBEPLDataSize];
            ...
            while (DataLen > 0)
            {
                ...
                if (SendCMD_EPLRead((uint)iAddr, (ushort)iCurReadNum) == false) return false;

                if (GetCDBStatus(ref errorStr, errorMsg) == false) return false;

                if (ReadEPLData(bytDataBuf, (ushort)iCurReadNum) == false) return false;
        ...
        public bool SendCDBCopy(ref string errorStr, string errorMsg = "", byte Direction = 0xBA)
        {
            if (SendCMD_Copy(Direction) == false || GetCDBStatus(ref errorStr, errorMsg) == false)
            {
                return false;
            }
            else
        ...
        public bool ReadCDBCopy(byte[] bytData, byte DataLen, ref string errorStr, string errorMsg = "")
        {
            if (ReadCopyData(bytData, DataLen) == false || GetCDBStatus(ref errorStr, errorMsg) == false)
            {
                return false;
            }
            else
