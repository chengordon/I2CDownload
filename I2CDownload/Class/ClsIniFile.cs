...
namespace I2CDownload
{
    public abstract class CustomIniFile
    {
        ...
        public string FileName
        {
            ...
            set 
            {
                string dirName = System.IO.Path.GetDirectoryName(value);
        ...
        public abstract ArrayList ReadSectionValues(string Section);
        public abstract bool ReadSection(string Section, List<string> Strings);
        public abstract ArrayList ReadSection(string Section);
        public abstract bool ReadSections(List<string> Strings);
        public abstract ArrayList ReadSections();
   ...
   /// <summary>
    /// 存储本地INI文件的类。
    /// </summary>
    public class ClsIniFile : CustomIniFile
    {
        [DllImport("kernel32")]
        ...
        /// </summary>
        /// 
        //public ClsIniFile(string AFileName)
        //    : base(AFileName)
        //{
        //}
        public ClsIniFile()
        ...
        public override ArrayList ReadSectionValues(string Section)
        {
            ArrayList arrayList = new ArrayList();
            string tmp;
            arrayList = ReadSection(Section);
            foreach (string vIdent in arrayList)
            {
                tmp = string.Format("{0}={1}", vIdent, ReadString(Section, vIdent, "")).Trim();
                if (tmp != "")
                {
                    arrayList.Add(tmp);
                }
        ...
        public override ArrayList ReadSection(string Section)
        {
            byte[] vBuffer = new byte[16384];
            uint vLength = GetPrivateProfileString(Section, null, null, vBuffer,
                (uint)vBuffer.Length, FileName);
            ...
            if (vLength>0)
            {
                for (int i = 0; i < vLength; i++)
                {
                    if (vBuffer[i] == 0)
                    {
                        tmp = Encoding.Default.GetString(vBuffer, j, i - j).Trim();
                        j = i + 1;
                        if (tmp != "")
                        {
                            arrayList.Add(tmp);
        ...
        public override ArrayList ReadSections()
        {
            byte[] vBuffer = new byte[16384];
            uint vLength = GetPrivateProfileString(null, null, null, vBuffer,
                (uint)vBuffer.Length, FileName);
